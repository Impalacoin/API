 ...
 * 
 */
public class QueryBalance2 extends HttpServlet {
	...
	private String checkBalance(HttpServletRequest request) throws IOException {
		...
		String username = "", sessionid = "",countrycode = "";
		...
		//country and country balance ip
	    for(Object key : keys){
	    	element = countryCache.get(key);
	    	country =(Country) element.getObjectValue();
	    	countryIp.put(country.getCountrycode(), country.getCountrybalanceip());
	    }
	    ...
	    if (!countryHash.containsKey(countrycode)) {
	    	...
	    	//retrieve the countryip to be used as URL
	  		String countrybalanceip = countryIp.get(countrycode);
	  		
	  		toairtel.put("api_username", username);
	        toairtel.put("session_id", "tyte5656");
	        toairtel.put("country_code", "254715290374");
			
			String jsonData = g.toJson(toairtel);
	  		
	  		
	   if(StringUtils.isNotEmpty(countrybalanceip)) {
	  		 
	  	    CLIENT_URL = countrybalanceip;
	  		...
	  		String switchbalance = roots.getAsJsonObject().get("balance")
						.getAsString();
